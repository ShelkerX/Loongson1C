...
int dgram_cnt, arp_cnt, ipv4_cnt;
int icmp_cnt, igmp_cnt, tcp_cnt, udp_cnt;
int dgram_cnt_pre;
static int fd = -1;
static int stat_fd = -1;
static int stat_buf[8];
static int stat_buflen = 32;
static int stat_st = 0;
static int stat_en = 0;
static void handle_ipv4(const char *ptr, rt_size_t buflen)
{
    unsigned char header_len = (*ptr) & 15;
    unsigned short tot_len = (((unsigned short)ptr[2]) << 8) | ptr[3];
    unsigned char proc = ptr[9];
    //printf("HEADER LEN %u TOT LEN %u\n", header_len, tot_len);
    switch (proc)
    {
    case 1: //ICMP
        icmp_cnt++;
...
static void write_to_file(int time)
{
    if (stat_fd < 0)
    {
        stat_fd = open("status.txt", O_WRONLY | O_CREAT | O_TRUNC, 0);
        if (stat_fd < 0)
        {
            dbg_log(DBG_ERROR, "open STAT file failed!\n");
            return;
        }
    }
    stat_buf[0] = time;
    stat_buf[1] = dgram_cnt;
    stat_buf[2] = arp_cnt;
    stat_buf[3] = ipv4_cnt;
    stat_buf[4] = icmp_cnt;
    stat_buf[5] = igmp_cnt;
    stat_buf[6] = udp_cnt;
    stat_buf[7] = tcp_cnt;
    int length = write(stat_fd, (char *)stat_buf, stat_buflen);
    //length = write(fd, buf, len);
    if (length != stat_buflen)
    {
        dbg_log(DBG_ERROR, "write STAT data failed, length: %d\n", length);
        close(fd);
        return;
    }
}
static void more_info(const rt_uint8_t *ptr, rt_size_t buflen)
{
    dgram_cnt++;
    unsigned char *buf = (unsigned char *)ptr;
    short eth_tp = *(short *)(buf + 12);
    //ptr += 14;
    switch (eth_tp)
    {
    case 0x68: //ARP
    ...
    case 0x08: //IPV4
        ipv4_cnt++;
        handle_ipv4(buf + 14, buflen);
        break;
    default:
        break;
    }
    //printf("NOW %d\n", stat_en);
    if (stat_en - stat_st >= 5)
    {
        /*
        if (dgram_cnt - dgram_cnt_pre >= 12)
        {
            dbg_log(DBG_ERROR, "real time flow exceed!!!\n");
        }*/
        dgram_cnt_pre = dgram_cnt;
        //printf("TIME %d CNT %d ARP %d IPV4 %d ICMP %d IGMP %d UDP %d TCP %d\n", stat_en, dgram_cnt, arp_cnt, ipv4_cnt, icmp_cnt, igmp_cnt, udp_cnt, tcp_cnt);
        stat_st = stat_en;
        write_to_file(stat_en);
    }
}
...
static void rt_tcpdump_ip_mess_write(struct tcpdump_buf *p)
{
    ...
    more_info(tbuf->buf, tbuf->tot_len);
#ifdef PKG_NETUTILS_TCPDUMP_PRINT
    ...
    while (1)
    {
        if (rt_mb_recv(tcpdump_mb, (rt_ubase_t *)&mbval, RT_WAITING_FOREVER) == RT_EOK)
        {
            ...
            if ((tcpdump_write != RT_NULL) && (tcpdump_write == rt_tcpdump_pcap_file_write))
            {
                ...
                stat_en = pkthdr.ts.tv_sec;
                tcpdump_write(&pkthdr, sizeof(pkthdr));
            }
            //more_info((rt_uint8_t *)&pkthdr, PCAP_PKTHDR_SIZE);
#ifdef PKG_NETUTILS_TCPDUMP_PRINT
